<script lang="ts">
	...
	const filteredOrders = $derived(
		filter === 'active'
			? data.orders.filter((o) => o.status !== 'collected' && o.status !== 'cancelled')
			: data.orders
	);
...
{#if filteredOrders.length === 0}
	<p class="mt-10 text-ink-soft">No orders here.</p>
{:else}
	<div class="mt-6 space-y-3">
		{#each filteredOrders as order (order.id)}
			<a
				href={`/admin/orders/${order.id}`}
				class="block rounded-xl border border-ink/10 bg-white/60 p-4 transition-colors hover:bg-white"
			>
				<div class="flex flex-wrap items-start justify-between gap-3">
					<div>
						<p class="font-medium text-ink">
							#{order.id} &middot; {order.guestName}
							<span class="ml-1 text-xs text-ink-soft">{formatDate(order.createdAt)}</span>
						</p>
						<p class="mt-0.5 text-xs text-ink-soft">
							{order.fulfilmentMethod === 'delivery' ? 'Delivery' : 'Pickup'}
							{formatPickupDate(order.pickupDate)} &middot;
							{order.items.reduce((sum, i) => sum + i.quantity, 0)} item{order.items.reduce(
					...
					<div class="flex items-center gap-2">
						<span
							class={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${order.paymentStatus === 'paid' ? 'bg-gold/20 text-gold-deep' : 'bg-ink/5 text-ink-soft'}`}
						>
							{order.paymentStatus === 'paid' ? 'Paid' : 'Unpaid'}
						</span>
						<span
							class={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${statusClasses[order.status]}`}
						>
							{statusLabels[order.status]}
						</span>
						<p class="w-16 shrink-0 text-right font-semibold text-ink">
